...
def mean_structured_similarity_index(
    ...
    """Calculates the Mean Structural Similarity Index (MSSIM)
    between two images.
    """
    K1, K2 = 0.01, 0.03
    C1, C2 = (K1 * L) ** 2, (K2 * L) ** 2
    ...
    def bgr2gray(img: np.ndarray) -> np.ndarray:
        # Standard Rec. 601 grayscale: 0.2989 * R + 0.5870 * G + 0.1140 * B
        # Input is BGRA/BGR: 0=B, 1=G, 2=R
        return (
...
def is_similar(
    ...
    Args:
        img1: The first image as a NumPy array (BGRA).
        img2: The second image as a NumPy array (BGRA).
        similarity_threshold: The threshold above which images
    ...
    similarity: float = mean_structured_similarity_index(
...
def take_screenshots() -> List[np.ndarray]:
    ...
    with mss.mss() as sct:
        ...
        for i in monitor_indices:
            ...
            # Return raw BGRA image to avoid unnecessary copy/conversion
...
def record_screenshots_thread() -> None:
    """Continuously records screenshots, processes them, and stores data."""
    ...
    while True:
        ...
        for i, current_screenshot in enumerate(current_screenshots):
            ...
            if not is_similar(current_screenshot, last_screenshot):
                ...
                # Convert to RGB for saving and OCR
                current_screenshot_rgb = current_screenshot[:, :, [2, 1, 0]]

                image = Image.fromarray(current_screenshot_rgb)
                timestamp = int(time.time())
                # Add monitor index to filename to prevent overwrites
                ...
                text: str = extract_text_from_image(current_screenshot_rgb)
                # Only proceed if OCR actually extracts text
                if text.strip():
                    embedding: np.ndarray = get_embedding(text)
                    active_app_name: str = (
                        get_active_app_name() or "Unknown App"
                    )
